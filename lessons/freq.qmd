...
Frequency is an important construct in many areas of language, and more generally, in human cognition (think: our pattern recognition abilities). Frequency affects which words and phrases are learned first, in L1s and L2s. More frequent grammatical constructions are learned first (and better for L2 speakers) than less frequent ones (e.g., active voice vs. passive voice in English). More frequency words experience phonetically-driven sound changes (e.g., lenition) first. More frequent word resist analogical change (*keep* -\> *\*keeped*, but *leap* -\> *leapt*, *leaped*).
...
The logic to calculate frequencies of files on your (2s impersonal) harddrive in R is simple:
...
2.  Ask R to count up the number of word tokens per word type in the vector.
    ...
    1.  There are two ways (Dr. Brown will show) to count up word tokens per word type:
        ...
        2.  Convert the vector into a one-column data frame and then use `count()`.
...
The output of the `table()` function is a named one-dimensional array (aka. vector), of class `table`. The values are the integers (i.e., the frequencies), and each integer has a name (i.e., the words). In order to extract only the names, you can use the `names()` function:
...
**Quick little aside**: the pipe operator `%>%` pipes (aka. passes) the value on the left-hand side of the operator into the function on the right-hand side, as the first argument to that function.
...
After a good-faith effort, if you need so help, take a look at Dr. Brown's code below:
