...
1.  Parse the files so that all words in all files are in a single vector with N elements (N being the total number of words \[specifically, word tokens\] across all files);

2.  Ask R to count up the number of word tokens per word type in the vector.

    1.  There are three ways (Dr. Brown will show) to count up word tokens per word type:

        1.  With the base R `table()` function;
        2.  Convert the vector into a one-column data frame and then use `count()` in `tidyverse`.
        3.  Using a `list` data structure, count of frequencies of words in one file at a time.
