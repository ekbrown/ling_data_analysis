...
Students will right R code that calls other languages.
...
If you know Python (and NLTK), you might remember that the output of this call in Python itself is a Python `list` of two-item `tuple`s. Differently, here we get an R `list` (which is different from a Python `list`). We could continue to process this R `list` as needed, for example, looping over it in a `for` loop to find particular part-of-speech tags:
