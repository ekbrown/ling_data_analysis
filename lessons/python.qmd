...
py_require("nltk")
nltk <- import("nltk")
tokens <- nltk$word_tokenize(txt); print(tokens)
...
Here's another example. Dr. Brown likes the [`num2words`](https://github.com/savoirfairelinux/num2words) Python package because it transcribes Arabic numerals to Romanized characters, for example, `4` to `four` (or `fourth` if you want the ordinal number). Let's call this Python package from within R:
```{r num2words}
#| eval: false
library("reticulate")
